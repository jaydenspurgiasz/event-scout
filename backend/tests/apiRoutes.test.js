...
Task: Generate a complete test suite using jest for the functions in routes/apiRoutes.js. Do not write any implementation or production code and do not assume any logic for the function. Leave a brief comment detailing the purpose of each test case.
...
describe("API Routes", () => {
  ...
  describe("Event Routes", () => {
    ...
    test("GET /api/event/search - get all events with auth", async () => {
      jwt.verify.mockReturnValue({ id: 1, email: "test@example.com" });
      db.getAllEvents.mockResolvedValue([
        { id: 1, title: "Event 1" },
        { id: 2, title: "Event 2" },
      ]);
    ...
    test("POST /api/event/:id/rsvp - fail without auth", async () => {
      const response = await request(app).post("/api/event/123/rsvp");
    ...
    test("DELETE /api/event/:id/rsvp - fail without auth", async () => {
      const response = await request(app).delete("/api/event/123/rsvp");
    ...
    test("GET /api/event/:id/participants - get participants without auth", async () => {
      ...
      const response = await request(app).get("/api/event/123/participants");
    ...
    test("GET /api/event/:id/participants - handle errors", async () => {
      db.getAllEventParticipants.mockRejectedValue(
    ...
    test("GET /api/event/attending/:id - fail without auth", async () => {
      const response = await request(app).get("/api/event/attending/2");
    ...
    test("GET /api/event/attending/:id - handle errors", async () => {
      jwt.verify.mockReturnValue({ id: 1, email: "test@example.com" });
      db.getEventsUserIsAttending.mockRejectedValue(
